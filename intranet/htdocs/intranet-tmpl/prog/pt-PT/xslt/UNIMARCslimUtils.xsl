...
<xsl:stylesheet version="1.0"
 ...
 <xsl:attribute name="title">Pesquisar pelo editor</xsl:attribute>
 <xsl:value-of select="."/>
 </a>
 <xsl:if test="position() != last()">
 <xsl:text>, </xsl:text>
 ...
 <span class="label">
 <xsl:choose>
 <xsl:when test="@ind2=1">Produção: </xsl:when>
 <xsl:when test="@ind2=2">Distribuição: </xsl:when>
 <xsl:when test="@ind2=3">Fabricante: </xsl:when>
 <xsl:when test="@ind2=4">
 <xsl:choose>
 <xsl:when test="substring(marc:subfield[@code='d'],1,1)='C'">Data de direitos de autor: </xsl:when>
 <xsl:when test="substring(marc:subfield[@code='d'],1,1)='P'">Data de proteção:</xsl:when>
 <xsl:otherwise>Data de copyright / data de proteção: </xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 <xsl:otherwise>Publicação: </xsl:otherwise>
 </xsl:choose>
 </span>
 <span>
 <xsl:call-template name="addClassRtl" />
 ...
 <span class="label">Informações de Impressão e/ou Publicação Transcritas como encontradas no Colofão: </span>
 ...
 <span class="label">Informações de Impressão e/ou Publicação Transcritas como encontradas na Fonte Principal de Informação: </span>
 ...
 <span class="label">Ligado a: </span>
 <span class="value">
 <xsl:call-template name="addClassRtl" />
